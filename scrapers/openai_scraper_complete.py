...
class OpenAIAuctionScraper:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        self.sources = [
            ...
            {
                "name": "Lone Star Auctioneers - Texas",
                "source_id": "lone_star",
                "base_url": "https://www.lonestarauctioneers.com/search/texas/",
                "source_url": "https://www.lonestarauctioneers.com",
                "type": "lone_star",
                "pages": [
                    "https://www.lonestarauctioneers.com/search/texas/",
                    "https://www.lonestarauctioneers.com/search/texas/?page=2"
                ]
            },
            {
                "name": "Rene Bates Auctioneers - Texas",
                "source_id": "rene_bates",
                "base_url": "https://www.renebates.com/",
                "source_url": "https://www.renebates.com",
                "type": "rene_bates",
                "pages": [
                    "https://www.renebates.com/",
                    "https://www.renebates.com/auctions/all?state=TX"
                ]
            }
        ]
    ...
    def _get_system_prompt(self, source_type: str) -> str:
        ...
        if source_type == "govdeals":
            ...
            Make sure to extract the complete URL to each auction's detail page for the "Visit Source" button.
            """
        elif source_type == "lone_star":
            return base_prompt + """
            For Lone Star Auctioneers, focus on extracting auctions from the search results.
            Look for auction listings with titles, descriptions, and images.
            Pay special attention to extracting the auction dates which may be in a specific format.
            Pay special attention to image elements and extract all image URLs, even if they're in data-src attributes.
            Make sure to extract the complete URL to each auction's detail page for the "Visit Source" button.
            """
        elif source_type == "rene_bates":
            return base_prompt + """
            For Rene Bates Auctioneers, focus on extracting auctions from the homepage and auction listings.
            Look for auction listings with titles, descriptions, and images.
            Pay special attention to extracting the auction dates which may be in a specific format.
            Pay special attention to image elements and extract all image URLs, even if they're in data-src attributes.
            Make sure to extract the complete URL to each auction's detail page for the "Visit Source" button.
    ...
    def _parse_date(self, date_str: str) -> Optional[str]:
        ...
        try:
            # Handle relative time formats like "1 hour 25 mins"
            relative_time_match = re.search(r'(\d+)\s+hour(?:s)?\s+(\d+)\s+min(?:s)?', date_str)
            if relative_time_match:
                hours = int(relative_time_match.group(1))
                minutes = int(relative_time_match.group(2))
                # Calculate end date based on current time plus the relative time
                end_date = datetime.now().replace(microsecond=0)
                end_date = end_date.replace(hour=end_date.hour + hours, minute=end_date.minute + minutes)
